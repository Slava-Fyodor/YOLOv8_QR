    ...
    def run(self):
        ...
        try:
            ...
            if self.prepare_yolo_compatible_dataset():
                self.log_update.emit("Оценка производительности модели с использованием стандартных методов валидации...")
    ...
    def load_model(self):
        ...
        try:
            ...
            if custom_names and hasattr(self.model, 'names'):
                self.log_update.emit(f"Использовать пользовательские имена классов для переопределения имен классов модели по умолчанию")
                if len(custom_names) < len(self.model.names):
                    self.log_update.emit(f"Warning：({len(custom_names)}) < ({len(self.model.names)})")
        ...
        for yaml_path in possible_yaml_paths:
            if os.path.exists(yaml_path):
                try:
                    ...
                    if 'names' in data:
                        self.log_update.emit(f"Загрузка имени класса из YAML-файла: {yaml_path}")
                        # 处理不同格式的names（列表或字典）
                    ...
                    self.log_update.emit(f"Не удалось прочитать имя класса из YAML: {str(e)}")
        ...
        for class_file in possible_class_files:
            if os.path.exists(class_file):
                try:
                    ...
                    if class_names:
                        self.log_update.emit(f"Загрузка имён классов из classes.txt: {class_file}")
                        return class_names
                except Exception as e:
                    self.log_update.emit(f"Не удалось прочитать имена классов из classes.txt: {str(e)}")
